...
One last function is defined:\begin{minted}{c}
...
Note that the choice to use arrays here is not innocent, since those definitions will be defined via their positions in this array (corresponding to their positions in the file) in the levels (see below).
...
\begin{minted}{c}
enum { // screens
    SCREEN_WELCOME, SCREEN_INSTRUCTIONS, SCREEN_GAME_OVER, SCREEN_WIN, SCREEN_SCORE,
    SCREEN_NUMBER
};
...
void game_loop() {
    ...
    if (time_spent < 1.0f / FPS)
        custom_usleep((useconds_t) ((1.0f / FPS - time_spent) * 1000));
}
\end{minted}
so that the function update the \cc{key_counters} array first (using \cc{keys_update_interval()}), and then call the input management and drawing function of the game or the screen, depending on \cc{paused} and \cc{current_screen} (if \cc{paused} is false). This function is therefore the main function of the program.
