...
%\end{minted}
%
%Indeed, when two players are present, each dragon (player) have its own score and number of life. Also, when a dragon die, if it still have some lives left, it comes back as invincible for a few second (the \cc{invicibility_counter} is decremented at each frame, until it gets to 0).
%
...
%\item A \cc{bool key_pressed[E_SIZE]} array. Values are set to \cc{true} when a keyboard key is pressed, and \cc{false} when released (using \cc{glutKeyBoardFunc()} to register the pressed keys and \cc{glutKeyboardUpFunc()} to register if they are released \cite{keyb}).
%\item A \cc{unsigned int key_pressed_interval[E_SIZE]}, which will be set to 0 when a key is pressed, and for which the value will increase at each frame if the key is not up. The goal is to keep track of the amount of time since a key has been pressed, and eventually repeat the event (for example, if the player maintain the direction key pressed) if \cc{key_pressed_interval[key] % FRAMES_BETWEEN_KEY_REPEAT == 0}.
%\end{enumerate}
%
